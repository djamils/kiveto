      ...
      <div class="hcard-row">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="display:flex;align-items:center;gap:8px;margin-top:3px;">
            <span style="font-size:var(--text-xs);color:var(--text-subtle);">${fmtDays(p.admission)}</span>
            <span style="font-size:var(--text-xs);color:var(--text-subtle);">· ${p.vet}</span>
          </div>
        </div>
        <div style="text-align:right;flex-shrink:0;">
          <p style="font-size:var(--text-sm);font-weight:var(--weight-medium);color:${done === total ? '#059669' : '#0f172a'};">${done}/${total}</p>
          <p style="font-size:var(--text-xs);color:var(--text-subtle);">soins</p>
        </div>
      </div>
      <!-- Progress bar -->
...
function renderDetail(p) {
  ...
  document.getElementById('detail-panel').innerHTML = `
    ...
    <div class="detail-header">
      <div style="display:flex;align-items:center;gap:14px;">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            <span style="font-size:var(--text-sm);color:var(--text-subtle);">${p.proprio} · ${p.tel}</span>
            <span style="font-size:var(--text-sm);color:var(--text-subtle);">· ${p.vet}</span>
            <span style="font-size:var(--text-sm);color:var(--text-subtle);">· Admis ${fmtDays(p.admission)}</span>
      ...
      ${p.notes ? `<div style="margin-top:10px;background:#f8fafc;border-radius:8px;padding:8px 12px;font-size:var(--text-sm);color:var(--text-muted);border-left:3px solid #4338ca;">${p.notes}</div>` : ''}
    </div>
    ...
    <div class="detail-tabs">
      ...
      <div class="dtab" data-tab="evo" onclick="setTab('evo')">
        Évolution
        <span style="font-size:var(--text-xs);background:#f8fafc;color:var(--text-muted);padding:0 6px;border-radius:10px;margin-left:5px;border:1px solid #e2e8f0;">${p.evolution.length}</span>
      </div>
  ...
  if (currentTab === 'soins') {
    ...
    el.innerHTML = `
      <div class="widget">
        ...
        ${pending.length ? `<div>${pending.map(s => soinRow(s, p)).join('')}</div>` : '<div class="wb"><p style="font-size:var(--text-md);color:var(--text-subtle);text-align:center;padding:8px 0;">✅ Tous les soins du jour ont été administrés</p></div>'}
      </div>
      ${done.length ? `
      <div class="widget">
      ...
      <div class="w2" style="margin-bottom:12px;">
        <div class="widget">
          ...
          <div class="wb">
            ...
            ${totalRepas === 0 ? `<div style="margin-top:8px;text-align:center;padding:8px;background:#f8fafc;border-radius:8px;font-size:var(--text-sm);color:var(--text-subtle);">Aucune alimentation prévue</div>` : ''}
          </div>
          ${a.repas.length ? `<div>${a.repas.map((r, i, arr) => `
            <div style="display:flex;align-items:center;gap:10px;padding:8px 14px;${i < arr.length - 1 ? 'border-bottom:1px solid #f1f5f9;' : ''}transition:background .12s;" onmouseenter="this.style.background='#f5f3ff'" onmouseleave="this.style.background='transparent'">
              ...
              <div style="flex:1;"><p style="font-size:var(--text-sm);font-weight:var(--weight-medium);color:var(--text-primary);">${r.type} · ${r.quantite}</p></div>
              <span style="font-size:var(--text-sm);color:var(--text-subtle);">${r.heure}</span>
          ...
          <div class="wb">
            ...
            <div style="margin-top:10px;">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;">
                <span style="font-size:var(--text-xs);color:var(--text-muted);font-weight:var(--weight-medium);">Hydratation estimée</span>
              ...
              <p style="font-size:var(--text-xs);color:var(--text-subtle);margin-top:3px;">Objectif indicatif : 50ml/kg/j (${Math.round(parseFloat(p.poids) * 50)} ml)</p>
            </div>
          </div>
        </div>
      </div>`;
            ...
            <div class="evo-entry">
              ...
              <div style="flex:1;min-width:0;padding-bottom:${i < arr.length - 1 ? '10px' : '0'};">
                <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:5px;">
                  ...
                  <span style="font-size:var(--text-sm);font-weight:var(--weight-medium);color:var(--text-secondary);">${e.date} à ${e.heure}</span>
                  <span style="font-size:var(--text-xs);color:var(--text-subtle);">${e.vet}</span>
                </div>
                <p style="font-size:var(--text-md);color:var(--text-secondary);line-height:1.5;margin-bottom:6px;">${e.note}</p>
                ${e.temp || e.fc || e.fr ? `
                <div style="display:inline-flex;gap:10px;background:#f8fafc;border:1px solid #e8edf2;border-radius:8px;padding:5px 10px;">
                  ${e.temp ? `<span style="font-size:var(--text-xs);color:var(--text-muted);">🌡 <strong>${e.temp}°C</strong></span>` : ''}
                  ${e.fc ? `<span style="font-size:var(--text-xs);color:var(--text-muted);">❤️ <strong>${e.fc} bpm</strong></span>` : ''}
                  ${e.fr ? `<span style="font-size:var(--text-xs);color:var(--text-muted);">🫁 <strong>${e.fr}/min</strong></span>` : ''}
                </div>` : ''}
              </div>
            </div>`).join('')}
        </div>
          ...
          <div class="wb">
            ${p.evolution[0] ? `
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;">
                ...
                <span style="font-size:var(--text-sm);color:var(--text-subtle);">${p.evolution[0].date} ${p.evolution[0].heure}</span>
              </div>
              <p style="font-size:var(--text-md);color:var(--text-secondary);line-height:1.5;">${p.evolution[0].note}</p>
            ` : '<p style="font-size:var(--text-sm);color:var(--text-subtle);">Aucune évolution saisie</p>'}
          </div>
        </div>
      </div>
...
function soinRow(s, p) {
  ...
  return `<div class="soin-row">
    ...
    <div style="flex:1;min-width:0;">
      ...
      <p style="font-size:var(--text-xs);color:var(--text-subtle);">${s.dose} · ${s.freq}</p>
    </div>
    <div style="text-align:right;flex-shrink:0;">
