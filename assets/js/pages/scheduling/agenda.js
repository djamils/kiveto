...
function createRdvBlock(a){
  ...
  if(lines <= 0){
    ...
    html += `<div style="${R}">
      ${dot}
      <span style="font-size:var(--text-xs);font-weight:var(--weight-bold);color:${timeColor};flex-shrink:0;">${fmtTime(a.start)} · ${a.durationMin}min</span>
    ...
    if(lines >= 2) html += `<div style="${R}">
      <span style="font-size:var(--text-xs);font-weight:var(--weight-bold);color:${nameColor};${strikethrough}overflow:hidden;text-overflow:ellipsis;">${animal?.emoji||''} ${animal?.name||'?'}</span>
    </div>`;
    // L3: owner
...
function initMotifFilters(){
  ...
  Object.entries(MOTIF_COLORS).forEach(([motif, color]) => {
    ...
    chip.innerHTML = `
      ...
      <span style="font-size:var(--text-sm);color:var(--text-secondary);flex:1;">${motif}</span>
...
function renderDayPlanning(){
  ...
  if(dayAppts.length === 0){
    list.innerHTML = `<p style="font-size:var(--text-xs);color:var(--text-subtle);text-align:center;padding:12px 0;">Aucun RDV ce jour</p>`;
    return;
  }
  ...
  list.innerHTML = dayAppts.map(a => {
    ...
    <div style="display:flex;align-items:flex-start;gap:8px;padding:7px 8px;border-radius:8px;background:#fff;border:1px solid #f1f5f9;cursor:pointer;transition:background .1s;${opacity}" onmouseenter="this.style.background='#f8fafc'" onmouseleave="this.style.background='#fff'">
      ...
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:var(--text-xs);color:var(--text-subtle);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${animal?.owner||''}</div>
      </div>
    </div>`;
  }).join('');
}
