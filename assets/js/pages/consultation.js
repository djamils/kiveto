...
function addPlanLine(){
  ...
  function finalizeLine(){
    ...
    var iconSpan=cfg?'<span style="font-size:10px;background:'+cfg.bg+';color:'+cfg.color+';padding:1px var(--space-1);border-radius:var(--radius-sm);font-weight:var(--weight-medium);flex-shrink:0;">'+cfg.icon+'</span>':'';
...
function addToOrdo(btn,name,detail){
  ...
  row.innerHTML='<div style="width:5px;height:5px;border-radius:50%;background:'+color+';flex-shrink:0;"></div><span data-med="'+name+'" style="font-size:var(--text-md);font-weight:var(--weight-medium);color:var(--text-primary);flex:1;">'+name+'</span><span style="font-size:10px;color:var(--text-muted);">'+(detail.split('·')[0]||'').trim()+'</span>';
