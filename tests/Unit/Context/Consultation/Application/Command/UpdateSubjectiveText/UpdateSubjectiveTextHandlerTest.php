...
namespace App\Tests\Unit\Context\Consultation\Application\Command\UpdateSubjectiveText;

use App\Context\Consultation\Application\Command\UpdateSubjectiveText\UpdateSubjectiveText;
use App\Context\Consultation\Application\Command\UpdateSubjectiveText\UpdateSubjectiveTextHandler;
...
final class UpdateSubjectiveTextHandlerTest extends TestCase
{
    ...
    private UpdateSubjectiveTextHandler $handler;

    protected function setUp(): void
    {
        $this->consultations = $this->createMock(ConsultationRepositoryInterface::class);
        $this->clock         = $this->createMock(ClockInterface::class);
        $this->handler       = new UpdateSubjectiveTextHandler($this->consultations, $this->clock);
    }

    public function testUpdateSubjectiveTextSuccessfully(): void
    {
        ...
        ($this->handler)($this->command('  Toux depuis 3 jours  '));

        self::assertSame('Toux depuis 3 jours', $consultation->getSubjectiveText());
    }

    public function testClearSubjectiveTextWithNull(): void
    {
        $consultation = $this->makeConsultation();
        $consultation->updateSubjectiveText('Ancien texte', new \DateTimeImmutable('2026-04-10 09:05:00'));
    ...
    public function testFailsWhenConsultationNotFound(): void
    {
        ...
        ($this->handler)($this->command('Toux'));
    }
    ...
    private function command(?string $text): UpdateSubjectiveText
    {
        return new UpdateSubjectiveText(
