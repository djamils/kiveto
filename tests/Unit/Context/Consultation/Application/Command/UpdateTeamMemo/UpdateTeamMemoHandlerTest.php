...
namespace App\Tests\Unit\Context\Consultation\Application\Command\UpdateTeamMemo;

use App\Context\Consultation\Application\Command\UpdateTeamMemo\UpdateTeamMemo;
use App\Context\Consultation\Application\Command\UpdateTeamMemo\UpdateTeamMemoHandler;
...
final class UpdateTeamMemoHandlerTest extends TestCase
{
    ...
    private UpdateTeamMemoHandler $handler;

    protected function setUp(): void
    {
        $this->consultations = $this->createMock(ConsultationRepositoryInterface::class);
        $this->clock         = $this->createMock(ClockInterface::class);
        $this->handler       = new UpdateTeamMemoHandler($this->consultations, $this->clock);
    }

    public function testUpdateTeamMemoSuccessfully(): void
    {
        ...
        ($this->handler)($this->command('  Chat difficile à manipuler  '));

        self::assertSame('Chat difficile à manipuler', $consultation->getTeamMemo());
    }

    public function testClearTeamMemoWithNull(): void
    {
        $consultation = $this->makeConsultation();
        $consultation->updateTeamMemo('Ancien mémo', new \DateTimeImmutable('2026-04-10 09:05:00'));
    ...
    public function testFailsWhenConsultationNotFound(): void
    {
        ...
        ($this->handler)($this->command('Mémo'));
    }
    ...
    private function command(?string $memo): UpdateTeamMemo
    {
        return new UpdateTeamMemo(
