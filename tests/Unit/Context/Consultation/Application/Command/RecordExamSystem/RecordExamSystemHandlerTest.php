...
namespace App\Tests\Unit\Context\Consultation\Application\Command\RecordExamSystem;

use App\Context\Consultation\Application\Command\RecordExamSystem\RecordExamSystem;
use App\Context\Consultation\Application\Command\RecordExamSystem\RecordExamSystemHandler;
...
final class RecordExamSystemHandlerTest extends TestCase
{
    ...
    private RecordExamSystemHandler $handler;

    protected function setUp(): void
    {
        $this->consultations = $this->createMock(ConsultationRepositoryInterface::class);
        $this->clock         = $this->createMock(ClockInterface::class);
        $this->handler       = new RecordExamSystemHandler($this->consultations, $this->clock);
    }

    public function testRecordExamSystemSuccessfully(): void
    {
        ...
        ($this->handler)($this->command('CARDIOVASCULAR', 'ANOMALY'));

        self::assertSame(['CARDIOVASCULAR=ANOMALY'], self::examsOf($consultation));
    }
    ...
    #[AllowMockObjectsWithoutExpectations]
    public function testFailsWhenBodySystemIsUnknown(): void
    {
        $this->consultations->expects(self::once())->method('findById')->willReturn($this->makeConsultation());
        $this->consultations->expects(self::never())->method('save');

        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('Unknown body system');

        ($this->handler)($this->command('TENTACLES', 'ANOMALY'));
    }

    #[AllowMockObjectsWithoutExpectations]
    public function testFailsWhenExamStatusIsUnknown(): void
    {
        $this->consultations->expects(self::once())->method('findById')->willReturn($this->makeConsultation());
        $this->consultations->expects(self::never())->method('save');

        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('Unknown exam status');

        ($this->handler)($this->command('CARDIOVASCULAR', 'WEIRD'));
    }

    private function command(string $system, string $status): RecordExamSystem
    {
        return new RecordExamSystem(
            consultationId: self::CONSULTATION_ID,
            clinicId: self::CLINIC_ID,
            system: $system,
            status: $status,
            notes: 'Souffle systolique',
            structuredData: ['murmur' => '2/6'],
     ...
     * @return list<string>
     */
    private static function examsOf(Consultation $consultation): array
    {
        return array_map(
            static fn (ExamSystemRecord $exam): string => $exam->getSystem()->value . '=' . $exam->getStatus()->value,
