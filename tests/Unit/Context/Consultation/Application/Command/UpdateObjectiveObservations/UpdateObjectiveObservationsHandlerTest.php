...
namespace App\Tests\Unit\Context\Consultation\Application\Command\UpdateObjectiveObservations;

use App\Context\Consultation\Application\Command\UpdateObjectiveObservations\UpdateObjectiveObservations;
use App\Context\Consultation\Application\Command\UpdateObjectiveObservations\UpdateObjectiveObservationsHandler;
...
final class UpdateObjectiveObservationsHandlerTest extends TestCase
{
    ...
    private UpdateObjectiveObservationsHandler $handler;

    protected function setUp(): void
    {
        $this->consultations = $this->createMock(ConsultationRepositoryInterface::class);
        $this->clock         = $this->createMock(ClockInterface::class);
        $this->handler       = new UpdateObjectiveObservationsHandler($this->consultations, $this->clock);
    }

    public function testUpdateObjectiveObservationsSuccessfully(): void
    {
        ...
        ($this->handler)($this->command('  Muqueuses roses  '));

        self::assertSame('Muqueuses roses', $consultation->getObjectiveObservations());
    }

    public function testClearObjectiveObservationsWithBlankText(): void
    {
        $consultation = $this->makeConsultation();
        $consultation->updateObjectiveObservations('Ancien texte', new \DateTimeImmutable('2026-04-10 09:05:00'));
    ...
    private function command(?string $text): UpdateObjectiveObservations
    {
        return new UpdateObjectiveObservations(
