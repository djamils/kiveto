...
namespace App\Tests\Unit\Context\Consultation\Application\Command\RecordTypedVital;

use App\Context\Consultation\Application\Command\RecordTypedVital\RecordTypedVital;
use App\Context\Consultation\Application\Command\RecordTypedVital\RecordTypedVitalHandler;
...
final class RecordTypedVitalHandlerTest extends TestCase
{
    ...
    private RecordTypedVitalHandler $handler;

    protected function setUp(): void
    {
        $this->consultations = $this->createMock(ConsultationRepositoryInterface::class);
        $this->clock         = $this->createMock(ClockInterface::class);
        $this->handler       = new RecordTypedVitalHandler($this->consultations, $this->clock);
    }

    public function testRecordTypedVitalSuccessfully(): void
    {
        ...
        ($this->handler)($this->command('HEART_RATE', '90'));

        self::assertSame(['HEART_RATE=90'], self::vitalsOf($consultation));
    }
    ...
    public function testFailsWhenVitalTypeIsUnknown(): void
    {
        ...
        ($this->handler)($this->command('MOON_PHASE', '90'));
    }

    private function command(string $type, string $value): RecordTypedVital
    {
        return new RecordTypedVital(
     ...
     * @return list<string>
     */
    private static function vitalsOf(Consultation $consultation): array
    {
        return array_map(
            static fn (TypedVitalRecord $vital): string => $vital->getType()->value . '=' . $vital->getValue(),
