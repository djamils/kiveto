...
namespace App\Tests\Unit\Context\Consultation\Application\Command\MarkAllSystemsNormal;

use App\Context\Consultation\Application\Command\MarkAllSystemsNormal\MarkAllSystemsNormal;
use App\Context\Consultation\Application\Command\MarkAllSystemsNormal\MarkAllSystemsNormalHandler;
...
final class MarkAllSystemsNormalHandlerTest extends TestCase
{
    ...
    private MarkAllSystemsNormalHandler $handler;

    protected function setUp(): void
    {
        $this->consultations = $this->createMock(ConsultationRepositoryInterface::class);
        $this->clock         = $this->createMock(ClockInterface::class);
        $this->handler       = new MarkAllSystemsNormalHandler($this->consultations, $this->clock);
    }

    public function testMarkAllSystemsNormalKeepsExistingAnomaly(): void
    {
        ...
        ($this->handler)($this->command(['CARDIOVASCULAR', 'RESPIRATORY']));

        self::assertSame(
            ['CARDIOVASCULAR=ANOMALY', 'RESPIRATORY=NORMAL'],
            self::examsOf($consultation),
        );
    }
    ...
    public function testFailsWhenConsultationNotFound(): void
    {
        ...
        ($this->handler)($this->command(['RESPIRATORY']));
    }
    ...
    public function testFailsWhenBodySystemIsUnknown(): void
    {
        ...
        ($this->handler)($this->command(['RESPIRATORY', 'TENTACLES']));
    }

    /**
     * @param list<string> $systems
     */
    private function command(array $systems): MarkAllSystemsNormal
    {
        return new MarkAllSystemsNormal(
