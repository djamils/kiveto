...
use App\Context\Procurement\Application\Command\ClosePurchaseOrder\ClosePurchaseOrder;
use App\Context\Procurement\Application\Command\ClosePurchaseOrder\ClosePurchaseOrderHandler;
...
final class ClosePurchaseOrderHandlerTest extends TestCase
{
    ...
    public function testItClosesReceivedPurchaseOrder(): void
    {
        $po = $this->makeReceivedPurchaseOrder();

        $repository = $this->createMock(PurchaseOrderRepositoryInterface::class);
        $repository->method('findById')->willReturn($po);
        $repository->expects(self::once())->method('save');

        ($this->makeHandler($repository))(new ClosePurchaseOrder(purchaseOrderId: self::PO_UUID));
    ...
    private function makeHandler(PurchaseOrderRepositoryInterface $repository): ClosePurchaseOrderHandler
    {
        ...
        return new ClosePurchaseOrderHandler(
            $repository,
    ...
    private function makeReceivedPurchaseOrder(): PurchaseOrder
    {
        ...
        $po->recordFullReception([self::LINE_UUID => '5'], new \DateTimeImmutable());
        $_ = $po->pullDomainEvents();
